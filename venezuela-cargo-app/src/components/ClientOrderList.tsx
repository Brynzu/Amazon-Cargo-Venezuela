...
const getStatusDisplay = (status: string) => {
  switch (status) {
    case 'awaiting_approval':
      return { text: 'Awaiting Approval', icon: Clock, color: 'text-gray-600', bg: 'bg-gray-100 border-gray-300' }
    case 'rejected':
      return { text: 'Rejected', icon: XCircle, color: 'text-red-600', bg: 'bg-red-50 border-red-200' }
...
export function ClientOrderList({ initialOrders, user }: { initialOrders: any[], user: any }) {
  ...
  const handlePaymentSubmit = async () => {
    ...
    try {
      ...
      setOrders(orders.map(o => o.id === payingOrder.id ? { ...o, receipt_url: publicUrl } : o))
      setPayingOrder(null)
      setFile(null)
    ...
    <>
      <div className="grid gap-4">
        {orders.map((order) => {
          ...
          const needsPayment = order.status === 'pending_payment' && !order.receipt_url
